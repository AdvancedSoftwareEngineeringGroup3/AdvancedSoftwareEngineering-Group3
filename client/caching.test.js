...
//   expect(await storeData("tester", testData)).toBe(0);
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(JSON.stringify("tester"), JSON.stringify(testData));
});
...
test('updates data in cache', async () => {
    ...
    expect(AsyncStorage.mergeItem).toHaveBeenCalledWith(JSON.stringify("tester"), JSON.stringify(changedData));
});


test('remove data from cache', async () => {
    await removeData("tester");
    expect(AsyncStorage.removeItem).toHaveBeenCalledWith(JSON.stringify("tester"));
});

test('handles errors when storing data to cache', async () => {
    AsyncStorage.setItem.mockRejectedValueOnce(new Error('Error storing data in cache'));
