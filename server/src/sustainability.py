...
class Sustainability:
    ...
    def db_update_monthly_distances(self, user, journey_data):
        """
        Updates monthly distances for each transport mode travelled by user

        Args:
            user (str): username of the user
            journey_data (dict): dictionary of distances travelled by user for each transport mode
        Returns:
    ...
    def db_fetch_raw_distances(self, user):
        """
        Fetches distances for each transport mode travelled by user
        for the current month
        Args:
            user (str): username of the user

        Returns:
            dict: dictionary of distances travelled by user for each transport mode
        """
        table_name = "monthly_distance"
        db = DataBase()
        db.connect_db()
    ...
    def db_fetch_all_sust_friends(self, user):
        """
        Fetches all users friends
        Args:
            user (str): username of the user
        Returns:
            list: list of friends usernames
        """
    ...
    def calc_scores_from_route(self, user, journey_data):
        """
        Calculates total sustainability score from route data
        Args:
            user (str): username of the user
            journey_data (dict): dictionary of distances travelled by user for each transport mode
        ...
        emissions_savings = self.calc_emissions_savings(journey_data)
        transport_score = 0
        ...
        for transport_mode in journey_data:
            ...
            # skip values not used in calculations
            if transport_mode == "car" or transport_mode == "total" or transport_mode == "username":
                continue

            transport_score += self.calc_scores(
                self.calc_emissions_savings[transport_mode]
            )
